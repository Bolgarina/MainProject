#include "./AsymFrustumProjectionMatrix.h"

namespace Math
{
	AsymFrustumProjectionMatrix::AsymFrustumProjectionMatrix(const float &i_left, const float &i_right,
		...
		const float &i_near, const float &i_far) : BaseMatrix(4)
	{
		...
		matrix[0][2] = 2.0f * (i_left + i_right) / (i_right - i_left);
		matrix[1][2] = 2.0f * (i_top + i_bottom) / (i_top - i_bottom);
